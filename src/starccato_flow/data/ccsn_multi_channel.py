...
from ..localisation.detector_network import DetectorNetwork
...
class CCSNDataMultiChannel(Dataset):
    ...
    def __init__(
        ...
        self.detector_network = DetectorNetwork()
        self.include_sky_params = include_sky_params
        
        # Initialize location handler
        self.ccsn_locations = CCSNLocations()
    ...
    def _project_to_detectors(self) -> np.ndarray:
        ...
        for i in range(n_samples):
            signal = self.signals[:, i]  # Shape: (Y_LENGTH,)
            ra, dec, distance = self.sky_params[i]
            
            # Scale by distance (inverse square law approximation)
            # Normalize to 10 kpc reference distance
            distance_factor = 10.0 / distance
            signal_scaled = signal * distance_factor
            
            for j, det_name in enumerate(self.detectors):
                # Compute antenna response patterns
                F_plus, F_cross = self.detector_network.compute_antenna_patterns(ra, dec, det_name)
                
                # Project signal to detector
                # For simplicity, assume optimal orientation (circular polarization)
                # Full version: h_det = F_plus * h_plus + F_cross * h_cross
                # Simplified: use geometric mean of F_plus and F_cross
                antenna_factor = np.sqrt(F_plus**2 + F_cross**2)
                
                multi_channel[i, j, :] = signal_scaled * antenna_factor
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, ...]:
        ...
        multi_signal = self.multi_channel_signals[original_idx]  # Shape: (num_detectors, Y_LENGTH)

...
class Sky:
    ...
    def generate_galactic_supernovae(self, num_supernovae):
        ...
        r_test = np.linspace(0, 16.8, 1000)
        pdf_max = np.max(np.abs(self.pdf(r_test, A, r_0, theta_0, beta)))
        
        # Rejection sampling to generate radial distances from the PDF
        r_samples = []
        while len(r_samples) < num_supernovae:
            # Propose samples uniformly in the range
            r_proposal = np.random.uniform(0, 16.8, num_supernovae * 2)
            # Generate uniform random values for acceptance test
            u = np.random.uniform(0, pdf_max, num_supernovae * 2)
            # Accept samples where u < pdf(r)
            accepted = r_proposal[u < np.abs(self.pdf(r_proposal, A, r_0, theta_0, beta))]
